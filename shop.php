      ...
      <div class="container">
        ...
        <div class="row element-animate">
          <div class="major-caousel js-carousel-1 owl-carousel">
            ...
            $products = mysqli_query($connect, "SELECT * FROM `products_toy`");
            ...
            foreach ($products as $product) {
              ...
              <div class="media d-block media-custom text-center">
                ...
                <h3 class="mt-0 product__name"><?= $product[1] ?></h3>
                <div class="mt-0 text-black"><?= $product[2] ?></div>
                <div class="mt-0 text-black"><?= $product[3] ?>&#8381;</div>
                <a href="basket.php?id=<?= $product[0] ?>" class="btn btn_shop">В корзину</a>
                <a href="comments_toy.php?id=<?= $product[0] ?>" class="btn btn_shop">Комментарии</a>
                  <?php if (!empty($_SESSION['admin']) && $_SESSION['admin'] == true):?>
                <a href="update.php?id=<?= $product[0] ?>" class="p-2 btn_cart"><span class="fa fa-pencil"></span></a>
                <a href="vendor/delete_toy.php?id=<?= $product[0] ?>" class="p-2  btn_cart"><span class="fa fa-trash"></span></a>
                 <?php endif; ?>
              </div>
              </div>
            </div>
...
<?php if (!empty($_SESSION['admin']) && $_SESSION['admin'] == true):?>
    <div class="container">
  <h3>Добавить новую игрушку</h3>
  <form action="vendor/create_toy.php" method="post">
    <p>Название</p>
...
<section class="section bg-light">
      <div class="container">
        ...
        <div class="row element-animate">
          <div class="major-caousel js-carousel-1 owl-carousel">
            ...
            $products_feed = mysqli_query($connect, "SELECT * FROM `products_feed`");

            /*
             * Преобразовываем полученные данные в нормальный массив
             */

            $products_feed = mysqli_fetch_all($products_feed);
            foreach ($products_feed as $product_feed) {
              ?>
              <div class="media d-block media-custom text-center">
              <img src="img/feed_1.png" alt="Image Placeholder" class="img-fluid">
              <div class="media-body">
                <h3 class="mt-0 product__name"><?= $product_feed[1] ?></h3>
                <div class="mt-0 text-black"><?= $product_feed[2] ?></div>
                <div class="mt-0 text-black"><?= $product_feed[3] ?>&#8381;</div>
                <a href="basket.php?id=<?= $product_feed[0] ?>" class="btn btn_shop">В корзину</a>
                <a href="comments_feed.php?id=<?= $product_feed[0] ?>" class="btn btn_shop">Комментарии</a>
                <?php if (!empty($_SESSION['admin']) && $_SESSION['admin'] == true):?>
                <a href="update_feed.php?id=<?= $product_feed[0] ?>" class="p-2  btn_cart"><span class="fa fa-pencil"></span></a>
                <a href="vendor/delete_feed.php?id=<?= $product_feed[0] ?>" class="p-2  btn_cart"><span class="fa fa-trash"></span></a>
                <?php endif; ?>
                 </div>
              </div>
            </div>
  ...
  <?php if (!empty($_SESSION['admin']) && $_SESSION['admin'] == true):?>
 <div class="container">
  <h3>Добавить новый корм</h3>
  <form action="vendor/create_feed.php" method="post">
    <p>Название</p>
    <input type="text" name="name">
    <p>Описание</p>
    <textarea name="description"></textarea>
